...
DEFAULT_METRICS = ("hel", "jsd", "csd", "hyb", "ssc")
...
def stein_scott_distance_matrix(query, library, mass_power=3.0, intensity_power=0.6, eps=1e-12):
    n_features = library.shape[1]
    mz_values = torch.arange(1, n_features + 1, dtype=library.dtype, device=library.device)

    mass_weight = mz_values.pow(mass_power)

    query_weighted = query.clamp_min(0).pow(intensity_power) * mass_weight
    library_weighted = library.clamp_min(0).pow(intensity_power) * mass_weight

    dot = torch.sum(library_weighted * query_weighted, dim=1)
    query_norm_sq = torch.sum(query_weighted.pow(2))
    library_norm_sq = torch.sum(library_weighted.pow(2), dim=1)

    similarity = dot.pow(2) / (library_norm_sq * query_norm_sq + eps)
    similarity = torch.clamp(similarity, 0.0, 1.0)

    return 1.0 - similarity


def stein_scott_pdist(data, mass_power=3.0, intensity_power=0.6, eps=1e-12):
    n_features = data.shape[1]
    mz_values = torch.arange(1, n_features + 1, dtype=data.dtype, device=data.device)

    mass_weight = mz_values.pow(mass_power)
    weighted = data.clamp_min(0).pow(intensity_power) * mass_weight

    norm = torch.norm(weighted, dim=1, keepdim=True) + eps
    normalized = weighted / norm

    similarity = torch.clamp(torch.matmul(normalized, normalized.T).pow(2), 0.0, 1.0)
...
def compute_metric_distances(query, library, lamb):
    ...
    return {
        ...
        "ssc": stein_scott_distance_matrix(query, library)
    }
