...
def find_candidate_set(query, library, tolerance):
    query = np.asarray(query, dtype=np.float32)
    library = np.asarray(library, dtype=np.float32)

    peak_order = np.argsort(query)[::-1]
    peak_order = peak_order[query[peak_order] > 0]
    ...
    for mz_idx in peak_order:
        ...
        current_mask = (
            (library[:, mz_idx] >= lower)
            & (library[:, mz_idx] <= upper)
        )
...
def evaluate_single_trial(query, library, target_idx, tolerances):
    ...
    for tolerance in tolerances:
        candidate_mask = find_candidate_set(
            query=query,
            library=library,
            tolerance=tolerance,
        )

        result[tolerance] = int(candidate_mask[target_idx])
...
def evaluate_candidate_screening_accuracy(data, noise_fn, tolerances=DEFAULT_TOLERANCES, n_trials=50):
...
def run_gaussian_noise_experiments(data, sigmas, tolerances, n_trials, rng):
    ...
    for sigma in sigmas:
        accuracy = evaluate_candidate_screening_accuracy(
            data=data,
